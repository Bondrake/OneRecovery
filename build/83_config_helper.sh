#!/bin/bash
#
# OneFileLinux Configuration Helper (83_config_helper.sh)
...
CACHE_DIR="${CACHE_DIR:-$HOME/.onefilelinux/cache}"
...
print_config() {
    ...
    if [ "${USE_CACHE:-true}" = "true" ]; then
        log "INFO" "  Cache directory: ${CACHE_DIR:-$HOME/.onefilelinux/cache}"
    fi
...
generate_module_env() {
    ...
    env_vars+="export CACHE_DIR=${CACHE_DIR:-$HOME/.onefilelinux/cache} "
    ...
    if [ "${USE_CACHE:-true}" = "true" ] && command -v ccache &> /dev/null; then
        env_vars+="export CCACHE_DIR=${CACHE_DIR:-$HOME/.onefilelinux/cache}/ccache "
...
setup_cache() {
    if [ "${USE_CACHE:-true}" = "true" ]; then
        log "INFO" "Setting up cache directory: ${CACHE_DIR:-$HOME/.onefilelinux/cache}"
        ...
        if command -v ccache &> /dev/null; then
            export CCACHE_DIR="${CACHE_DIR:-$HOME/.onefilelinux/cache}/ccache"
